...
checkIfEshaHasEnd({String eshaEndTime='', required PageController pageController,required String startTime,
  required String endTime,}){
   DateTime now = DateTime.now();
  final nowAMorPM = DateFormat('a').format(now);
  if(checkPrayerTime(endTime:endTime, startTime: startTime )==false){
    if(nowAMorPM=='PM'){
      if(eshaEndTime.isNotEmpty){
        ...
        if(now.formatted() == generate()[pageController.page?.round()??0].formatted()){
            final formattedEndEshaTime = checkForCorrectFormattingEndTime(endTime);
        if(now.hour==formattedEndEshaTime.hour && now.minute>=formattedEndEshaTime.minute){
